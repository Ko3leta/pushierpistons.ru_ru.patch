...
By contributing to Pushier Pistons, you agree with the [Developer Certificate of Origin (DCO)][DCO].
...
Use American English to keep consistency across Pushier Pistons.

If there is more than one acceptable spelling of the same word, first check if one word is already
being used in Pushier Pistons, FrozenLib, or in Mojang's Mappings, and if not, use the spelling that is most commonly used.
...
Fields marked as `@Unique` must be prefixed with `pushierPistons$` or `PUSHIERPISTONS$` if the field is static and final.

Methods marked with `@Unique` must be prefixed with `pushierPistons$`.

Injector or modifier methods do not need to be prefixed. Fabric takes care of that.

## Licensing & DCO

Pushier Pistons is licensed under [FrozenBlock Modding Oasis License][LICENSE], and it has a [Developer Certificate of Origin (DCO)][DCO], which you are required to agree with to contribute.
...
[LICENSE]: ./LICENSE.md "Pushier Pistons license file"
